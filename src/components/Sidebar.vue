<template>
    <div class="h-full flex flex-col bg-white border-r border-[#e5e7eb]">
        ...
        <div class="sidebar-logo flex items-center p-[10px] gap-[10px] shrink-0 whitespace-nowrap">
            <el-image :src="iconUrl" alt="logo" class="w-[50px] h-[50px] shrink-0" />
            <div class="logo-text" :class="{ collapse: isCollapse }">
        ...
        <el-menu
            ...
            collapse-transition
        >
            <el-menu-item v-for="item in backendRoute?.children" :key="item.path" :index="`${backendRoute.path}/${item.path}`">
                <el-icon>
